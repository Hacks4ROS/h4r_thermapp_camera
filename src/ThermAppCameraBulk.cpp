...
bool ThermAppCameraBulk::openDevice()
{
    ...
    for (size_t idx = 0; idx < count; ++idx)
    {
        ...
        if(desc.idVendor==0x1772 && desc.idProduct==0x0002)
        {
            ...
            int len=0;
            uint8_t dataDown=1;
        	retVal=libusb_bulk_transfer(handle, ENDPOINT_DOWN, &dataDown, 0, &len, 10);
        	if(retVal)
        	{
            	cerr<<__PRETTY_FUNCTION__<<"Error in transfer"<<
            	endl<<"Close interface!"<<endl;
            	libusb_close(handle);
        	}
...
void ThermAppCameraBulk::requestImage(cv::Mat& image)
{
	...
	unsigned char requestHeader[64] =
	{
		0xa5, 0xa5, 0xa5, 0xa5, 0xa5, 0xa5, 0xd5, 0xa5,	0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00,
	...
	{
			actual_length=0;
			libusb_bulk_transfer(handle, ENDPOINT_UP, requestHeader,
					64, &actual_length, 30);
			actual_length=0;
				...
				if(im_data.size()==384*288*2)
				{
					...
					for (int row = 0; row < 288; ++row)
					{
						for (int col = 0; col < 384; ++col)
						{
							...
							image.at<cv::Vec3b>(row,col)[0]=myuv.at<unsigned short>(row,col)/100;
					...
					image.at<cv::Vec3b>(288/2,384/2)[0]=0;
					image.at<cv::Vec3b>(288/2,384/2)[1]=0;
					image.at<cv::Vec3b>(288/2,384/2)[2]=0;
					cv::circle(image,cv::Point(288/4,384/2),10,cv::Scalar(255,0,0));
